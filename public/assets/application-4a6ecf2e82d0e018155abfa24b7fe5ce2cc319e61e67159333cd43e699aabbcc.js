  ...
  App.tables = App.cable.subscriptions.create("TablesChannel", {
    ...
    received: function(data) {
      ...
      if (active_table.length > 0) {
        return active_table.append(data.message);
      } else {
        return $("[data-behavior='table-link'][data-table-id='" + data.table_id + "']").css("font-weight", "bold");
      }
    }
  });
...
(function() {
  ...
  $(document).on("turbolinks:load", function() {
    return $("#new_die_roll").on("submit", function(ev) {
      ev.preventDefault();
      return $(this).submit();
    });
  });
